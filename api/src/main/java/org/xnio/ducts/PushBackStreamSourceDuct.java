 ...
 */
public final class PushBackStreamSourceDuct extends AbstractStreamSourceDuct<StreamSourceDuct> implements StreamSourceDuct {
    private StreamSourceDuct current = next;
    private boolean shutdown;

    public PushBackStreamSourceDuct(final StreamSourceDuct next) {
        super(next);
    }
    ...
    public void pushBack(Pooled<ByteBuffer> pooledBuffer) {
        ...
        if (shutdown || ! pooledBuffer.getResource().hasRemaining()) {
            ...
            current = new BufferDuct(current, pooledBuffer);
        }
    }

    class BufferDuct extends AbstractStreamSourceDuct<StreamSourceDuct> implements StreamSourceDuct {

        private final Pooled<ByteBuffer> pooledBuffer;

        BufferDuct(final StreamSourceDuct next, final Pooled<ByteBuffer> pooledBuffer) {
            super(next);
        ...
        public int read(final ByteBuffer dst) throws IOException {
            ...
            final StreamSourceDuct next = this.next;
            try {
                ...
                current = next;
                pooledBuffer.free();
                if (cnt > 0 && next == PushBackStreamSourceDuct.this.next) {
        ...
        public long read(final ByteBuffer[] dsts, final int offs, final int len) throws IOException {
            ...
            try {
                ...
                current = next;
                pooledBuffer.free();
                if (cnt > 0L && next == PushBackStreamSourceDuct.this.next) {
                    // don't hit the main channel until the user wants to
                    return cnt;
                }
            } catch (IllegalStateException ignored) {
