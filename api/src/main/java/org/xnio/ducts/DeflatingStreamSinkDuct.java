 ...
 */
public final class DeflatingStreamSinkDuct extends AbstractStreamSinkDuct<StreamSinkDuct> implements StreamSinkDuct {

    private static final byte[] NO_BYTES = new byte[0];
    private final Deflater deflater;
    private final ByteBuffer outBuffer;

    public DeflatingStreamSinkDuct(final StreamSinkDuct next, final Deflater deflater) {
        super(next);
    ...
    public long transferFrom(final FileChannel src, final long position, final long count) throws IOException {
        return src.transferTo(position, count, new WritableByteChannel() {
            public int write(final ByteBuffer src) throws IOException {
                return DeflatingStreamSinkDuct.this.write(src);
            }
