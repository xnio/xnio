...
import org.xnio.ssl.openssl.OpenSSLCipherConfigurationParser;
...
abstract class AbstractAcceptingSslChannel<C extends ConnectedChannel, S extends ConnectedChannel> implements AcceptingChannel<C> {
    ...
    AbstractAcceptingSslChannel(final SSLContext sslContext, final AcceptingChannel<? extends S> tcpServer, final OptionMap optionMap, final Pool<ByteBuffer> socketBufferPool, final Pool<ByteBuffer> applicationBufferPool, final boolean startTls) {
        ...
        openSSLCipherSuites = optionMap.get(Options.OPENSSL_ENABLED_CIPHER_SUITES);
    ...
    private static final Set<Option<?>> SUPPORTED_OPTIONS = Option.setBuilder()
            ...
            .add(Options.OPENSSL_ENABLED_CIPHER_SUITES)
    ...
    public <T> T setOption(final Option<T> option, final T value) throws IllegalArgumentException, IOException {
        if (option == Options.SSL_CLIENT_AUTH_MODE) {
            ...
            final String seq = Options.OPENSSL_ENABLED_CIPHER_SUITES.cast(value);
    ...
    public C accept() throws IOException {
        ...
        if (cipherSuites != null || openSSLCipherSuites != null) {
            ...
            } else if (openSSLCipherSuites != null) {
                final List<String> configuredCiphers = OpenSSLCipherConfigurationParser.parseExpression(openSSLCipherSuites);
                for (String name : configuredCiphers) {
                    if (supported.contains(name)) {
                        finalList.add(name);
                    }
                }
    ...
    public <T> T getOption(final Option<T> option) throws IOException {
        ...
        } else if (option == Options.OPENSSL_ENABLED_CIPHER_SUITES) {
