...
/**
 * AwaitAcceptable on a server that has reached its limit of connections.
 * After awaitAcceptable has checked that CONN_FULL is set, close the connections.
 * 
 * @author <a href="mailto:flavia.rainone@jboss.com">Flavia Rainone</a>
 */
@RunWith(BMUnitRunner.class)
@BMScript(dir="src/test/resources")
public class UnsetConnFullDuringAwaitTestCase {
    ...
    public void timeoutAwait() throws Exception {
        ...
        try {
            ...
            while (true) {
                ...
                channel.configureBlocking(false);
                channel.socket().bind(new InetSocketAddress(0));//new InetSocketAddress(Inet4Address.getByAddress(new byte[] { 127, 0, 0, 1 }), SERVER_PORT));
                channel.connect(new InetSocketAddress(Inet4Address.getByAddress(new byte[] { 127, 0, 0, 1 }), SERVER_PORT));
                final ConnectedStreamChannel connectedChannel = server.accept();
                if (connectedChannel == null) {
                    break;
