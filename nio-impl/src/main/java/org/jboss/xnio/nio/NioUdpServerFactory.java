 ...
 *
 */
public final class NioUdpServerFactory extends AbstractConfigurable implements ConfigurableFactory<BoundServer<SocketAddress, UdpChannel>> {
    ...
    NioUdpServerFactory(final NioXnio xnio, final Executor executor, final IoHandlerFactory<? super UdpChannel> factory, final SocketAddress[] initialAddresses) {
        super(options);
        this.xnio = xnio;
    ...
    public BoundServer<SocketAddress, UdpChannel> create() throws IOException {
        synchronized (lock) {
            ...
            final NioUdpServerConfig config = new NioUdpServerConfig();
            config.setXnio(xnio);
            ...
            final NioUdpServer udpServer = NioUdpServer.create(config);
