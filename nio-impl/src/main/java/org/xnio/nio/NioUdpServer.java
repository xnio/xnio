...
import org.xnio.management.UdpServerMBean;
...
class NioUdpServer implements UdpServer {
    ...
    private static final Logger log = Logger.getLogger("org.xnio.nio.udp.server");
    ...
    private volatile ChannelListener<? super MulticastMessageChannel> bindListener = null;
    ...
    private final ChannelListener.Setter<MulticastMessageChannel> bindSetter = IoUtils.getSetter(this, bindListenerUpdater);
    ...
    NioUdpServer(final NioXnio nioXnio, final Executor executor, final ChannelListener<? super MulticastMessageChannel> bindListener, final OptionMap optionMap) {
        synchronized (lock) {
    ...
    public ChannelListener.Setter<MulticastMessageChannel> getBindSetter() {
        return bindSetter;
    }
    ...
    public Collection<MulticastMessageChannel> getChannels() {
        synchronized (lock) {
            return new ArrayList<MulticastMessageChannel>(boundChannels);
        }
    }

    public IoFuture<MulticastMessageChannel> bind(final InetSocketAddress address) {
        synchronized (lock) {
            try {
                if (closed) {
                ...
                executor.execute(new Runnable() {
                    public void run() {
                        try {
                            IoUtils.<MulticastMessageChannel>invokeChannelListener(udpSocketChannel, bindListener);
                ...
                return new FailedIoFuture<MulticastMessageChannel>(e);
            }
        }
    }
